...
def fetch_cftc():
    """
    Scarica il report TFF settimanale da CFTC.gov.
    Salva il file con la DATA INTERNA del report (data del martedì),
    non con la data di oggi — così è sempre corretto indipendentemente
    da quando gira il workflow.
    """
    print(f"    GET {CFTC_URL}")
    try:
        r = get(CFTC_URL)
        r.raise_for_status()
        text = r.text.lstrip("\ufeff")
        ...
        for row in reader:
            ...
            # Leggi la data dal report (è uguale per tutte le righe)
            if report_date is None:
                raw_date = row.get("As_of_Date_In_Form_YYMMDD", "")
                if raw_date:
                    report_date = parse_cftc_date(raw_date)
            ...
            for label, code in COT_MARKETS.items():
                ...
                if code_matches(cftc_code, code) or code in market_name:
                    try:
                        ll = int(str(row.get("Lev_Money_Positions_Long_All",  "0")).replace(",", ""))
                        ls = int(str(row.get("Lev_Money_Positions_Short_All", "0")).replace(",", ""))
                        al = int(str(row.get("Asset_Mgr_Positions_Long_All",  "0")).replace(",", ""))
                        as_ = int(str(row.get("Asset_Mgr_Positions_Short_All","0")).replace(",", ""))
                        results[label] = {
                            "market":       market_name,
        ...
        print(f"    → {len(results)}/{len(COT_MARKETS)} mercati · data report: {report_date}")
        return {"date": report_date or TODAY, "source": CFTC_URL, "markets": results}, report_date
